...
export default {
  ...
  computed: {
    gradientColors() {
      return (this.siteStore.getSettings?.gradients || []).concat(this.gradientColorsDefault);
    },
    colorsPreview() {
    ...
    alpha() {
      const alpha = Math.round(
        Number(this.processColors[this.selectedColorIndex].color.split(",")[3].replace(")", "")) * 100
      );
      return alpha;
    }
  },
...
<template>
  <div class="color-gradient">
    ...
    <div class="color-gradient-container">
      ...
      >
        <div class="color-gradient-preview-action" v-if="gradientType == 'linear'">
          <div class="color-gradient-preview-overlay"></div>
          <div class="color-gradient-preview-slider" :style="{ transform: `rotate(${degree}deg)` }"></div>
          <div class="color-gradient-preview-degree">{{ degree }}</div>
        </div>
        <div class="color-gradient-preview-radial" v-if="gradientType == 'radial'">
          <div
      ...
      <div class="color-gradient-change">
        <div class="color-gradient-process">
          ...
          <div class="color-gradient-flip-color" @mousedown.prevent="handleFlipColor">
          ...
          <div class="color-gradient-delete-color" @mousedown.prevent="handleDeleteColor">
      ...
      <div class="color-gradient-custom" :style="{ '--color': processColors[selectedColorIndex].color }">
        <div
          ref="opacityColor"
      ...
      <div class="color-gradient-add">
        <div class="color-gradient-add-title">
          ...
          <div class="color-gradient-add-title-add" @mousedown.prevent="handleAddTemplate">+ Add</div>
        </div>
        ...
        <div class="color-gradient-add-preview">
          ...
          >
            <div class="color-gradient-add-preview-item-inner" :style="{ 'background-image': gradient }"></div>
          </div>
        </div>
      </div>
    </div>
...
.color-gradient-preview::before {
  ...
  left: 0;
  z-index: -1;
  background-image: repeating-conic-gradient(from 0deg, #fff 0 25%, #ededed 0 50%);
...
.color-gradient-preview-slider {
  ...
  box-shadow: var(0 0 2px 0 rgba(14, 15, 17, 0.3), inset 0 0 1px 0 rgba(14, 15, 17, 0.3));
...
.color-gradient-processing-node {
  ...
  box-shadow: var(--handle-selected-shadow, 0 1px 3px 0 rgba(23, 25, 28, 0.5));
  border: var(--handle-selected-border, solid 4px #fff);
  transform: translate(var(--handle-translate-x, 0), var(--handle-translate-y, 0));
  position: absolute;
...
.color-gradient-custom {
  ...
  align-items: center;
  justify-content: space-between;
  --range-input-track-background: linear-gradient(90deg, transparent, var(--color));
...
.color-gradient-opacity-range {
  ...
  width: 16px;
  height: 16px;
  transform: translateX(calc(var(--range-input-value) * var(--range-input-width) - 8px));
  border-radius: 50%;
...
.color-gradient-preview-conic-slider {
  ...
  border-radius: 50%;
  box-shadow: var(--angle-slider-shadow, 0 0 3px rgba(0, 0, 0, 0.3), inset 0 0 2px rgba(0, 0, 0, 0.1));
  opacity: 0;
...
.color-gradient-preview-conic-pointer::before {
  ...
  border-radius: 50%;
  left: calc(
    50% - calc(var(--knob-pointer-inner-diameter, 4px) * 1.6) + calc(var(--position-slider-diameter, 10px) / 2)
  );
...
.color-gradient-preview-conic-pointer::after {
  left: calc(50% + calc(var(--angle-slider-diameter, 60px) / 2) - calc(var(--knob-pointer-outer-diameter, 50px) / 1.5));
